...
# Repos created after 2026-07-15 get immutable-ID subject claims from GitHub
# (repo:owner@ownerid/name@repoid:...) instead of the old name-based
# repo:owner/name:... form -- the two are not interchangeable, and Azure
# matches the subject string exactly, so guessing wrong here means every OIDC
# login fails with AADSTS700213. Ask the API which one this repo actually
# produces rather than assuming.
REPO_OWNER_ID=$(gh api "repos/${REPO}" --jq .owner.id)
REPO_ID=$(gh api "repos/${REPO}" --jq .id)
REPO_OWNER=${REPO%%/*}
REPO_NAME=${REPO##*/}
REPO_SUBJECT="${REPO_OWNER}@${REPO_OWNER_ID}/${REPO_NAME}@${REPO_ID}"
...
add_federated_credential "github-environment-infra" "repo:${REPO_SUBJECT}:environment:infra"
add_federated_credential "github-pull-request"      "repo:${REPO_SUBJECT}:pull_request"
